...
# Test if coef, vcov and fitted work ------------------------------------------
...
test_that("simple methods work", {
  test_simple_method(coef, c(3,1))
  test_simple_method(vcov, c(3, 3))
  test_simple_method(fitted, n)
})

# Test for correct "nobs" in log likelihood -----------------------------------
...
test_that("simulate() works", {
  ...
  x <- simulate(m, seed = 969898)
  y <- simulate(m, seed = 969898)
  expect_equal(x, y)
