...
#' and [lasso_lmls()] functions are provided.
#'
...
predict.lmls <- function(object,
  ...
  if (length(predictor) > 1) {
    out <- lapply(predictor, function(p) {
      predict(object, newdata = newdata, p, type = type)
    })
...
#' Summary for LMLS
#'
#' Prints a summary for location-scale regression models from the [lmls()] and
#' [lasso_lmls()] function.
#'
#' @param object A location-scale regression model from the [lmls()] function.
#' @param type `"ml"`:
...
summary.lmls <- function(object,
  ...
  if (object$lasso){
    cat("Chosen Lambda values:\n")
    cat(c("Location:", round(unlist(object$lambda)[1], 3), "/", "Scale:", round(unlist(object$lambda)[2], 3)))
    cat("\n\n")
  }

  if (length(coef(object, "location"))) {
    cat("Location coefficients (identity link):\n")
    printCoefmat(coefmat(object, predictor = "location"), digits = digits, ...)
  } else {
    cat("No location coefficients\n")
  }
  ...
  if (length(coef(object, "scale"))) {
    cat("Scale coefficients (log link):\n")
    printCoefmat(coefmat(object, predictor = "scale"), digits = digits, ...)
  } else {
    cat("No scale coefficients\n")
  }
